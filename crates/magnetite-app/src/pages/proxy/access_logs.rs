//! Proxy access logs (S-PROXY-06). A proxy-scoped view of the shared `log`
//! table, populated by the embedded reverse proxy (Phase E3). The cross-cutting
...
#[component]
pub fn AccessLogsPage() -> impl IntoView {
    let reload = RwSignal::new(0_u32);
    let logs = Resource::new(
        move || reload.get(),
        |_| query_logs("proxy".to_string(), "access".to_string(), String::new()),
    );

    view! {
        <PageHeader title=Signal::derive(|| "アクセスログ".to_string())/>
        <ProxyNav/>
        <Suspense fallback=|| view! { <LoadingState/> }>
            {move || logs.get().map(|res| match res {
                Err(_) => view! { <ErrorState on_retry=Callback::new(move |_| reload.update(|n| *n += 1))/> }.into_any(),
                Ok(list) if list.is_empty() => view! { <EmptyState message=Signal::derive(|| "アクセスログはまだありません。".to_string())/> }.into_any(),
                Ok(list) => {
